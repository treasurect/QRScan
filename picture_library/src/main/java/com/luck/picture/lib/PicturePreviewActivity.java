...
public class PicturePreviewActivity extends PictureBaseActivity implements
  ...
  protected void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    rightTv.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View v) {
        LocalMedia imageSel = images.get(viewPager.getCurrentItem());
        Log.e("====", "onClick" + selectImages.size());
        if (selectImages.size() != 0 && !selectImages.get(0).getPath().equals(imageSel.getPath())) {
          Toast.makeText(mContext, "只能选择一个图片或视频", Toast.LENGTH_SHORT).show();
        } else {
          detailSelect();
        }
      }
  ...
  private void detailSelect() {
    ...
    if (config.enableCrop && pictureType.startsWith(PictureConfig.IMAGE)) {
      ...
      if (selectImages.size() == 0) {
        LocalMedia imageSel = images.get(viewPager.getCurrentItem());
        selectImages.add(imageSel);
      }
      toResult(selectImages);
    }
  }
  ...
  public void toResult(List<LocalMedia> images) {
    // 如果开启了压缩，先不关闭此页面，PictureImageGridActivity压缩完在通知关闭
    if (images.size() != 0 && images.get(0).getPictureType().startsWith(PictureConfig.IMAGE)) {
    ...
    if (images.size() != 0) {
      ...
      scaleVideo(images.get(0).getPath(), new VideoCompressListener() {
        ...
        public void onFailed(String errorMsg) {
          Toast.makeText(PicturePreviewActivity.this, "视频压缩失败，请重新选择", Toast.LENGTH_SHORT).show();
        }
      });
    }
